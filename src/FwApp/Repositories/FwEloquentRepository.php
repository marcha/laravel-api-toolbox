...
abstract class FwEloquentRepository
{
    ...
    public function getWithPagination(array $options = [])
    {
        ...
        $paginated = $query->paginate(intval($options['limit']));
    ...
    public function lookup($keyField, $displayField, $selectedId, $q){
        ...
        $data['list']=[];
        if ($selectedId){
            $data['selected'] =  $this->model->select([$keyField, $displayField])->where($keyField,'=',$selectedId)->first();
        }
